predict_maxent <- function(object,newdata,...){
  ...
  maxv <- eval_lamb(lambvals = lambvals$lambdas_df,bg_vars = newdata) - lambvals$maxmeta$linearPredictorNormalizer
  raw_max <- exp(maxv)/lambvals$maxmeta$densityNormalizer
  return(raw_max)
}
