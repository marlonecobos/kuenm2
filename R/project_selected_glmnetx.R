...
# #'
# #' @description
# #' This function predicts selected models across multiple scenarios, as specified in a `prepared_proj` object created wth the `prepare_proj()` function. In addition to generating predictions for each replicate, the function calculates cnsensus measures (e.g., mean, median) across replicates and models.
# #'
# #' @param models an object of class `fitted_models` returned by the `fit_selected_glmnetmx()` function.
# #' @param projection_data an object of class `prepared_proj` returned by the `prepare_proj()` function. This file cntains the paths to the rasters representing each scenario.
# #' @param out_dir (character) a path to a root directory for saving the raster file of each projection.
# #' @param consensus_per_model (logical) whether to calculate consensus across replicates when there are more than one rplicate per model. Default is TRUE.
# #' @param consensus_general (logical) whether to calculate consensus across models when there are more than one selected mdel. Default is TRUE.
# #' @param consensus (character) consensus measures to calculate. Options available are 'median', 'range', 'mean' and 'tdev' (standard deviation). Default is c("median", "range", "mean", "stdev").
# #' @param write_replicates (logical) whether to write the projections for each replicate. Default is FALSE.
# #' @param clamping (logical) whether to restricts variable values to the range of the calibration data to avoid etrapolation. Default is `TRUE` (free extrapolation).
# #' @param var_to_clamp (character) vector specifying which variables to clamp. Only applicable if `clamping = TRUE`. Dfault is `NULL`, meaning all variables will be clamped.
...
# #' @param overwrite (logical) whether to overwrite SpatRaster if they already exists. Only applicable if `write_files` is st to TRUE. Default is FALSE.
...
# #'
# #' @return A `model_projections` object that provides the paths to the raster files with the projection results and the crresponding thresholds used to binarize the predictions.
# #'
# #' @usage project_selected_glmnetx(models, projection_data,
# #'                                out_dir,
...
# #' out_dir <- file.path(tempdir(), "Projection_results")
# #' dir.create(out_dir, recursive = TRUE)
# #'
# #' # Project selected models for multiple scenarios
# #' p <- project_selected_glmnetx(models = fm,
# #'                               projection_data = pr,
# #'                               out_dir = out_dir,
# #'                               consensus_per_model = TRUE,
# #'                               consensus_general = TRUE,
...
#             .options.snow = opts
#             ,
#             .export = c("predict_selected_glmnetmx", "multiple_projections")
#     ) %dopar% {
