...
#' data with `prepare_data`. Only applicable if `include_m` is TRUE.
...
#' @export
#'
#' @usage explore_calibration_hist(data, include_m = FALSE,
...
explore_calibration_hist <- function(data, include_m = FALSE,
  ...
  if(!inherits(include_m, "logical")){
    stop("'include_m' must be 'logical', not ", class(include_m))
  }
  if(!inherits(magnify_occurrences, "numeric")){
    stop("'magnify_occurrences' must be 'numeric', not ",
         class(magnify_occurrences))
  }
  if(!inherits(breaks, "numeric")){
    stop("'breaks' must be 'numeric', not ", class(breaks))
  }
  ...
  if(include_m){
    ...
    if(!inherits(raster_variables, "SpatRaster"))
      stop("'raster_variables' must be a 'SpatRaster' object, not ",
           class(raster_variables))

  r_out <- setdiff(terra::names(raster_variables),
                   names(data$calibration_data)[-1])
  if(length(r_out) >= 1)
    stop("Some variables in data are absent from raster_variables")
  }
  ...
  var_hist <- lapply(v, function(i) {
    ...
    new_explore_back(hist_m = hist_m,
                     hist_bg = hist_bg,
                     hist_pr = hist_pr,
                     mean_m = m_m,
                     mean_bg = m_bg,
